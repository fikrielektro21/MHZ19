#ifndef MHZ19_ARDUINO_H
#define MHZ19_ARDUINO_H
...
/**
 * @brief Concrete implementation of MHZ19_Platform_Interface specifically for Arduino (AVR, Mega, Nano, Uno)
 * Uses Stream (can be HardwareSerial or SoftwareSerial)
 */
class MHZ19_Arduino_Platform : public MHZ19_Platform_Interface {
    private:
        Stream* _serial;
        uint32_t _timeout_ms;

    public:
        explicit MHZ19_Arduino_Platform(Stream* serial, uint32_t timeout_ms = 1000)
            : _serial(serial), _timeout_ms(timeout_ms) {}

        bool uart_transmit(const uint8_t* data, uint16_t length) override {
            if (!_serial) return false;
            while (_serial->available()) _serial->read(); // Bersihkan sisa data RX
...
/**
 * @brief Concrete implementation of MHZ19_PWM_Platform_Interface for standard Arduino (AVR)
 * Note: attachInterrupt on Arduino AVR (Uno/Nano) is typically
 * ONLY available on pins D2 and D3 (Interrupt 0 and 1).
 */
class MHZ19_PWM_Arduino_Platform : public MHZ19_PWM_Platform_Interface {
    private:
        uint8_t _pin = 255;
        ...
        static MHZ19_PWM_Arduino_Platform*& instancePtr() {
            static MHZ19_PWM_Arduino_Platform* ptr = nullptr;
            return ptr;
        }
        
        static void isrHandler() {
            MHZ19_PWM_Arduino_Platform* self = instancePtr();
            if (!self) return;
